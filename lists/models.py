...
class User(AbstractUser):
    def get_or_create_munch_log(self):
        ...
        munch_log, created = MunchLog.objects.get_or_create(
            owner=self, defaults={"name": f"{self.username}'s Munch Log"}
        )
        return munch_log
...
class Restaurant(models.Model):
    ...
    osm_type = models.CharField(
        max_length=1, choices=OSMType.choices, help_text="OpenStreetMap element type"
    )
...
class RestaurantImage(models.Model):
    ...
    alt_text = models.CharField(
        max_length=200, blank=True, help_text="Alt text for accessibility"
    )
    added_by = models.ForeignKey(
        User, on_delete=models.RESTRICT, help_text="User who added this image"
    )
...
class MunchLog(models.Model):
    ...
    owner = models.OneToOneField(
        User, on_delete=models.RESTRICT, related_name="munch_log"
    )
...
class ListComment(models.Model):
    restaurant_list = models.ForeignKey(
        RestaurantList, on_delete=models.CASCADE, related_name="comments"
    )
    author = models.ForeignKey(User, on_delete=models.RESTRICT)
    content = models.TextField(
        max_length=1000, help_text="Comment about this restaurant list"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class ListFollow(models.Model):
    follower = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="following_lists"
    )
    restaurant_list = models.ForeignKey(
        RestaurantList, on_delete=models.CASCADE, related_name="followers"
    )
